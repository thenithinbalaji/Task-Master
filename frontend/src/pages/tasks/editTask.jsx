...
import { fetchProject, validate, fetchUsers, fetchTask, updateTask } from "./../../utils/apiUtils";
...
export default function AddTask() {
    ...
    React.useEffect(() => {
        (async () => {
            try {
                ...
                console.log("(edit task page useEffect to fetch project)", status, response);
                ...
                console.log("(edit task page useEffect to fetch project) error", error);
                toast.error(error);
    ...
    React.useEffect(() => {
        (async () => {
            ...
            console.log("(edit task page useEffect to validate)", status, response);
    ...
    React.useEffect(() => {
        (async () => {
            if (projectData && loggedInUserID) {
                console.log("(edit tasks page useEffect to check admin) projectData", loggedInUserID, projectData.OwningUserID);
                if (loggedInUserID !== projectData.OwningUserID) {
                    toast.error("You are not authorized to edit tasks in this project");
                    navigate("/projects");
                }
    ...
    React.useEffect(() => {
        (async () => {
            try {
                const [response, status] = await fetchTask(pid, tid);
                console.log("(edit task page useEffect to fetch task)", status, response.message);
                ...
                if (status === 200) {
                    ...
                    toast.error("Task you are trying to fetch does not exist");
                    navigate(`/projects/${pid}`);
                }
            }

            catch (error) {
                console.log("(edit task page useEffect to fetch task) error", error);
                toast.error(error);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        console.log("(edit task page form submit)", pid, form.name, form.description, form.priority, form.assignedusers);
        ...
        try {
            ...
            const [response, status] = await updateTask(pid, tid, form.name, form.description, form.priority, form.assignedusers);
            console.log("(edit task page form submit)", status, response);
            if (status === 200) {
            ...
            console.log("(edit task page form submit)", error);
    ...
    return (loading ? <Loading /> :
        <div className="h-screen w-full p-14">
            <p className="mb-4 text-3xl font-extrabold leading-none tracking-tight text-gray-600 md:text-5xl lg:text-4xl">
                Edit Task {tid} in {projectData?.Name}
            </p>
            ...
            <form className="mt-14 w-full" onSubmit={handleSubmit}>
                ...
                <div className="w-full">
                    ...
                    <div className="flex flex-wrap">
                        ...
                        {form.assignedusers.map((email, index) => (
                            <div key={index} className="bg-gray-200 rounded-full px-2 py-1 m-1">
